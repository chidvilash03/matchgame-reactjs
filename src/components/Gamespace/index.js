...
class Gamespace extends Component {
  ...
  game = id => {
    const {matchId, score, timer} = this.state
    if (id === matchId && timer > 0) {
      this.setState(prevState => ({score: prevState.score + 1}))
      console.log(score)
      const number = Math.floor(Math.random() * imagesList.length)
      this.setState({
        matchId: imagesList[number].id,
        matchUrl: imagesList[number].imageUrl,
      })
    } else {
      clearInterval(this.id)
      this.setState({isRunning: false})
  ...
  render() {
    ...
    const {activeTabId, score, timer, matchUrl, isRunning} = this.state
    const updList = imagesList.filter(each => each.category === activeTabId)
    return (
      <div>
        <Navbar score={score} timer={timer} />
        {isRunning ? (
          <div className="main-container">
            <img src={matchUrl} alt="match" className="match" />
            ...
            <ul className="thu-container">
              {updList.map(each => (
                <Thumbnail imgList={each} game={this.game} key={each.id} />
              ))}
            </ul>
          </div>
        ) : (
